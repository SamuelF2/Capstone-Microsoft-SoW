...
from .llm_client import classify_section, extract_entities, extract_risks_llm, extract_deliverables_llm
...
def _detect_methodology(content: str) -> str:
    c = content.lower()
    scores = {
        "agile": sum(c.count(k) for k in ["sprint", "backlog", "scrum", "iteration", "product owner"]),
        "waterfall": sum(c.count(k) for k in ["phase gate", "requirements phase", "design phase", "uat"]),
        "sure-step-365": sum(c.count(k) for k in ["sure step", "fit-gap", "dynamics", "diagnostic phase"]),
        "cloud-adoption": sum(c.count(k) for k in ["landing zone", "migration wave", "azure caf", "workload assessment"]),
    }
...
def _classify(heading: str, content_preview: str) -> dict:
    ...
    return {"type": _heuristic_section_type(heading), "confidence": 0.6, "new_type": None, "reasoning": "heuristic"}


def _is_sow(raw_text: str) -> bool:
    lower = raw_text.lower()
    signals = sum(1 for kw in [
        "statement of work", "in scope", "out of scope", "acceptance criteria",
        "customer responsibilities", "deliverable", "prepared for", "prepared by",
    ] if kw in lower)
    return signals >= 3
...
def _write_entities(driver: Driver, extracted: dict, filename: str, parent_id: str, parent_label: str):
    id_map: dict[str, str] = {}
    ...
    for rel in extracted.get("relationships", []):
        ...
        if not from_id or not to_id:
            continue
        rel_type = re.sub(r"[^A-Z0-9_]", "_", rel.get("type", "RELATED_TO").upper().replace(" ", "_"))
        ...
        try:
            with driver.session() as session:
                session.run(
                    ...
                    aid=from_id, bid=to_id,
                )
        except Exception as e:
            logger.debug(f"Relationship write failed: {e}")


def _write_section(driver: Driver, sec: dict, sow_id: str, filename: str, banned_phrases: list[dict]):
    ...
    if section_type == "other" and classification.get("new_type"):
        ...
        with driver.session() as session:
            session.run(
                ...
                pid=_stable_id(new_type, "prop"), label=new_type, conf=conf, src=filename, acc=conf >= 0.8,
            )
    ...
    with driver.session() as session:
        session.run(
            ...
            """,
            sid=sec_id, heading=sec["heading"], stype=section_type,
            content=sec["content"][:2000], cc=sec["char_count"], level=sec["level"],
            conf=classification["confidence"], reason=classification.get("reasoning", "")[:200],
            sow_id=sow_id,
        )
        if section_type != "other":
            session.run(
                "MATCH (sec:Section {id: $sid}) MERGE (ct:ClauseType {type_id: $tid}) MERGE (sec)-[:INSTANCE_OF]->(ct)",
                sid=sec_id, tid=section_type,
            )
        ...
        for risk in risks:
            ...
            with driver.session() as session:
                session.run(
                    ...
                    """,
                    rid=risk_id, sow_id=sow_id, desc=risk["description"][:500],
                    sev=risk.get("severity", "medium"), mit=risk.get("mitigation", "")[:500],
                    hm=risk.get("has_mitigation", False), cat=risk.get("category", "Delivery"),
        ...
        for deliv in deliverables:
            ...
            with driver.session() as session:
                session.run(
                    ...
                    """,
                    did=d_id, sow_id=sow_id, title=deliv["title"][:200],
                    desc=deliv.get("description", "")[:500],
                    ac=deliv.get("acceptance_criteria", "")[:500],
                    hac=deliv.get("has_ac", False), conf=float(deliv.get("confidence", 1.0)),
                )
            if deliv.get("has_ac"):
                for phrase in _check_banned_phrases(deliv.get("acceptance_criteria", ""), banned_phrases):
                    with driver.session() as session:
                        session.run(
                            "MATCH (d:Deliverable {id: $did}) MATCH (b:BannedPhrase {phrase: $phrase}) MERGE (d)-[:CONTAINS_BANNED_PHRASE]->(b)",
                            did=d_id, phrase=phrase,
                        )
...
def _write_sow(driver: Driver, doc: dict, banned_phrases: list[dict]):
    ...
    console.print(f"  [dim]→ SOW:[/] [yellow]{filename}[/] | methodology=[cyan]{methodology}[/] | sections={len(doc['sections'])}")

    with driver.session() as session:
        session.run(
            "MERGE (s:SOW {id: $sid}) SET s.title=$title, s.filename=$fn, s.methodology=$meth, s.char_count=$cc, s.format=$fmt, s.source='ingested'",
            sid=sow_id, title=doc["title"], fn=filename,
            meth=methodology, cc=len(doc["raw_text"]), fmt=doc["metadata"].get("format", "unknown"),
        )
        session.run(
            "MATCH (s:SOW {id: $sid}) MERGE (m:Methodology {method_id: $mid}) MERGE (s)-[:USES_METHODOLOGY]->(m)",
            sid=sow_id, mid=methodology,
        )
    ...
    for sec in doc["sections"]:
        ...
        with driver.session() as session:
            session.run(
                ...
                tid=ct_id, heading=sec["heading"][:200], content=sec["content"][:2000],
                fn=filename, stype=classification["type"],
            )
...
def ingest_file(driver: Driver, path: Path, doc_type: Optional[str] = None, banned_phrases: Optional[list[dict]] = None):
...
def ingest_directory(driver: Driver, data_dir: Path, banned_phrases: Optional[list[dict]] = None):
