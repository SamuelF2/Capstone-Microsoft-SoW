...
def ingest(data_dir: str, clear: bool):
    ...
    console.print(Panel.fit(
        "[bold]SOW Knowledge Graph Ingestion[/]\n"
        f"Source: [cyan]{data_path}[/]",
        title="SOW-KG"
    ))
...
def validate(sow_id: str):
    ...
    if results["banned_phrases"]:
        ...
        t.add_column("Section"); t.add_column("Phrase", style="red")
        t.add_column("Severity"); t.add_column("Suggestion", style="dim")
    ...
    if results["missing_sections"]:
        ...
        t.add_column("Section", style="red"); t.add_column("Error")
    ...
    if results["deliverables_missing_ac"]:
        console.print(f"[red]{len(results['deliverables_missing_ac'])} deliverable(s) missing acceptance criteria[/]")
    else:
        console.print("[green]All deliverables have acceptance criteria[/]")

    if results["risks_without_mitigation"]:
        console.print(f"[red]{len(results['risks_without_mitigation'])} risk(s) without mitigation[/]")
    else:
...
def approval(value: float, margin: float):
    ...
    t = Table()
    t.add_column("Stage"); t.add_column("Approver")
    t.add_column("Required"); t.add_column("Reason", style="dim")
    for a in result["approvers"]:
        t.add_row(a["stage"], a["display_name"],
                  "Yes" if a["required"] else "No", a["reason"])
    console.print(t)
    driver.close()
...
@click.option("--role", required=True,
              type=click.Choice(["solution-architect", "delivery-manager",
                                  "cpl", "cdp", "sqa-reviewer"]))
def checklist(role: str):
    ...
    t.add_column("ID"); t.add_column("Required")
    t.add_column("Category"); t.add_column("Item")
    for item in items:
...
def risks(sow_id: str):
    ...
    if risks_data:
        ...
        t.add_column("Severity"); t.add_column("Mitigated")
        t.add_column("Description"); t.add_column("Mitigation", style="dim")
        for r in risks_data:
            t.add_row(r["severity"].upper(),
    ...
    if triggered:
        ...
        t2.add_column("Section"); t2.add_column("Trigger", style="red")
        t2.add_column("Severity"); t2.add_column("Suggestion", style="dim")
        for r in triggered:
...
def similar(sow_id: str):
    ...
    t.add_column("Title"); t.add_column("Methodology")
    t.add_column("Shared Clauses", justify="right"); t.add_column("Outcome")
    for r in results:
        t.add_row(r["title"], r["methodology"],
                  str(r["shared_clauses"]), r["outcome"] or "unlabeled")
    console.print(t)
    driver.close()
...
@click.option("--index", default="section_embeddings",
              type=click.Choice([
                  "section_embeddings", "deliverable_embeddings",
                  "risk_embeddings",    "rule_embeddings",
                  "clausetype_embeddings",
              ]),
              show_default=True, help="Which vector index to search")
