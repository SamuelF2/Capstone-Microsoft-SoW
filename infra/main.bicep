...
module api 'modules/container-app.bicep' = {
  ...
  params: {
    ...
    env: [
      ...
      { name: 'NEO4J_URI', value: 'bolt://${neo4j.outputs.fqdn}:80' }
      { name: 'NEO4J_USER', value: 'neo4j' }
      { name: 'NEO4J_PASSWORD', secretRef: 'neo4j-password' }
      { name: 'POSTGRES_HOST', value: postgres.outputs.fqdn }
      { name: 'POSTGRES_PORT', value: '80' }
      { name: 'POSTGRES_DB', value: postgresDbName }
      { name: 'POSTGRES_USER', value: postgresUser }
      { name: 'POSTGRES_PASSWORD', secretRef: 'postgres-password' }
      { name: 'DATABASE_URL', value: 'postgresql://${postgresUser}:${postgresPassword}@${postgres.outputs.fqdn}:80/${postgresDbName}' }
