...
### First-deploy gotchas (observed 2026-05-13)

None of these are blockers but every one of them bit us during the 2026-05-13 demo-prep deploy. Worth a scan before the next owner runs `azd up` on a fresh subscription.

| Symptom | Likely cause | Fix |
|---|---|---|
| `azd deploy ingestion` fails with `unable to find a resource tagged with 'azd-service-name: ingestion'` even though the Container Apps Job exists and is tagged correctly | `azd` ≤ 1.23.5 doesn't iterate Container Apps Jobs as deploy targets — only Container Apps. Fixed in 1.24+ | `winget upgrade Microsoft.Azd` to ≥ 1.24.0. Workaround on older azd: `az acr build --registry crnrflxor4bm2jw --image cocoon/ingestion-capstone:<tag> --file Dockerfile.ingestion .` then `az containerapp job update --name caj-ingest-<token> --image <full-uri>` to point the Job at the new tag |
| Web returns 404 (or a stale bundle that's missing recent components) right after a successful-looking `azd deploy web` | The web Container App's default resources are **0.25 vCPU / 0.5 GiB**. Because `frontend/Dockerfile` runs `npm run build` in `CMD` (so `NEXT_PUBLIC_*` env vars bind at the right time), the Next.js build needs that memory at startup, and 0.5 GiB OOMs with `Next.js build worker exited with code: null and signal: SIGKILL` ~14 s in. With no `.next/` produced, `npm run start` either 404s or never gets reached | Live patch: `az containerapp update --name ca-web-<token> --cpu 2.0 --memory 4Gi --min-replicas 1`. Permanent fix: bump the default `cpu`/`memory` in `infra/modules/container-app.bicep` for the web service (≥ 2 GiB), and consider keeping `minReplicas: 1` for the demo window so cold-starts don't re-trigger the build every time |
| New deploy looks successful but the page still serves stale chunks (different hashes from what the new image would produce) | A previously-active revision is still `Active: True` at `trafficWeight: 0`. Container Apps' ingress falls back to it when the latest revision's replica isn't ready yet (e.g. still building or OOMing). Single-revision mode does not auto-deactivate old revisions | After every deploy: `az containerapp revision list --name <app> -o table`, then `az containerapp revision deactivate --revision <old>` for anything `Active=True` that isn't the one you just shipped |
| Backend `/health` shows `postgres: error: 'NoneType' object has no attribute 'acquire'` after a partial deploy | Postgres password drift: the api Container App was redeployed and got the current `AZURE_POSTGRES_PASSWORD` from azd env, but the postgres container's data dir still has the password from its original init. Lifespan logs show six retries of `password authentication failed for user "cocoon"`, then the app gives up and `database.pg_pool` stays `None` | `azd deploy postgres` (re-inits with the current env password; wipes the ephemeral data dir), then `azd deploy api` (fresh lifespan retries the connection). Documented under §12 too |
| First user signs in and admin-only pages 403 with `"System admin access required"` | `backend/auth.py` defaults to `role = "consultant"` on first INSERT, and `_OVERRIDABLE_ROLES` intentionally excludes `system-admin` (security — so the `X-Role-Override` header can never self-escalate). After every Postgres wipe, every account starts at `consultant` again | Promote directly: `az containerapp exec --name ca-postgres-<token> --resource-group <rg> --command "sh -c \"psql -U cocoon -d cocoon -c 'UPDATE users SET role = \$tag\$system-admin\$tag\$ WHERE email = \$tag\$<email>\$tag\$'\""`. The `$tag$` PostgreSQL dollar-quoting sidesteps the nested quote-escape required by `az containerapp exec` |
